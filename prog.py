...
def thread2():   
    ...
    while(1):
        # Ingreso de comandos. Formato: 'NNN C+{comando}'.
        if msvcrt.kbhit() and msvcrt.getwch() =='C':
            ...
            Desplegable_entrada = ttk.Combobox(Cuadro1,width=17,
                                               values=Lista_ID,
            ...
            #------------------------------------------------------------------------------
            boton = tk.Button(ventana,text='Enviar CMD',font='consolas 16 bold',bg='orange',command=captura)
...
while(True):
    try:
        Dato = s.recvfrom(1024)             # Espera hasta recibir dato desde algún Cliente.
